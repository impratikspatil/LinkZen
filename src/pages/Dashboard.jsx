...
import UrlFormCard from "../components/UrlFormCard";
...
function Dashboard() {
  ...
  const handleShareQr = async () => {
    try {
      await navigator.share({
        ...
        text: `QR Code for ${selectedLink.shortCode}`,
        url: qrCodeUrl,
      });
    } catch (error) {
      console.log(error);
  ...
  const directTraffic = Math.round(totalTraffic * 0.7);
  const socialTraffic = Math.round(totalTraffic * 0.2);
  const searchTraffic = totalTraffic - directTraffic - socialTraffic;

  const directPercentage =
    totalTraffic === 0 ? 0 : Math.round((directTraffic / totalTraffic) * 100);

  const socialPercentage =
    totalTraffic === 0 ? 0 : Math.round((socialTraffic / totalTraffic) * 100);

  const searchPercentage =
    totalTraffic === 0 ? 0 : Math.round((searchTraffic / totalTraffic) * 100);

  const handleDeleteUrl = async () => {
    try {
      await deleteUrl(selectedLink.shortCode);
      setRecentLinks((prev) =>
        prev.filter((link) => link.shortCode !== selectedLink.shortCode)
      );
  ...
  const handleUpdateExpiry = async () => {
    ...
    try {
      const updatedUrl = await updateExpiry(
        selectedLink.shortCode,
        Number(expiryDays)
      );
      setRecentLinks((prev) =>
        prev.map((link) =>
          link.shortCode === updatedUrl.shortCode ? updatedUrl : link
        )
      );
    ...
    <div className="relative min-h-screen bg-black text-white overflow-hidden">
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-6 py-16">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-14">
          ...
          <div>
            <p className="text-gray-400 mb-3">Welcome Back 👋</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-14">
          ...
          <div className="bg-white/5 border border-white/10 rounded-3xl p-8 backdrop-blur-xl">
            <p className="text-gray-400 mb-3">Total Links</p>
            <h2 className="text-5xl font-bold">{recentLinks.length}</h2>
          </div>

          <div className="bg-white/5 border border-white/10 rounded-3xl p-8 backdrop-blur-xl">
            <p className="text-gray-400 mb-3">Total Clicks</p>
            <h2 className="text-5xl font-bold">{totalTraffic}</h2>
          </div>

          <div className="bg-white/5 border border-white/10 rounded-3xl p-8 backdrop-blur-xl">
            <p className="text-gray-400 mb-3">Active Links</p>
            <h2 className="text-5xl font-bold">
              {recentLinks.filter((link) => !link.expired).length}
            </h2>
          </div>

          <div className="bg-white/5 border border-white/10 rounded-3xl p-8 backdrop-blur-xl">
            <p className="text-gray-400 mb-3">Expired Links</p>
            <h2 className="text-5xl font-bold">
              {recentLinks.filter((link) => link.expired).length}
            </h2>
          </div>
        ...
        <div className="flex flex-col gap-8 mb-14">
          ...
          {/* Create URL Form */}
          <div id="create-link">
            <UrlFormCard onSuccess={fetchUrls} />
          </div>
          ...
          <div className="bg-white/5 border border-white/10 rounded-3xl p-8">
            ...
            <div className="flex items-center justify-between mb-8">
              <div>
                <h2 className="text-3xl font-bold mb-2">Recent URLs</h2>
                <p className="text-gray-400">Manage and track your generated links</p>
              </div>
            </div>
              ...
              <table className="w-full">
                ...
                <thead className="border-b border-white/10">
                  <tr className="text-left">
                    <th className="pb-5">Short URL</th>
                    <th className="pb-5">Original URL</th>
                    <th className="pb-5">Clicks</th>
                    <th className="pb-5">Expires In</th>
                        ...
                        <td className="py-5 w-[140px]">
                          {link.expiresAt ? (
                            (() => {
                              ...
                              return diffDays > 0 ? (
                                <span className="text-green-400">{diffDays} days</span>
                              ) : (
            ...
            <div className="space-y-6">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <p className="text-gray-300">Direct</p>
                  <p className="text-gray-400">{directPercentage}%</p>
                </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <p className="text-gray-400">{socialPercentage}%</p>
                </div>
                <div className="w-full h-3 bg-white/10 rounded-full overflow-hidden">
                  <div
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <p className="text-gray-400">{searchPercentage}%</p>
                </div>
                <div className="w-full h-3 bg-white/10 rounded-full overflow-hidden">
                  <div
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
          <div className="bg-[#111827] border border-white/10 rounded-3xl p-8 w-[90%] max-w-md">
            <h2 className="text-2xl font-bold mb-4">Delete Link</h2>
            <p className="text-gray-400 mb-8">
              Are you sure you want to delete
              <span className="text-white font-semibold"> {selectedLink?.shortCode}</span>?
            </p>
      ...
      {showEditModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
          <div className="bg-[#111827] border border-white/10 rounded-3xl p-8 w-[90%] max-w-md">
            <h2 className="text-2xl font-bold mb-4">Edit Expiry</h2>
